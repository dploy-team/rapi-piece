...
import { Graphics, AnimationInstance, ActorLoader } from "./deps/flare";
...
export class FlarePlayer implements AnimationPlayer {
  ...
  private advance(time?: number) {
    ...
    if (
      this.playing &&
      (!this.animation ||
        this.position < this.animation.duration ||
        this.animation.loop)
    ) {
    ...
    if (actorInstance) {
      ...
      if (actorInstance._Animations.length) {
        ...
        if (this.animationName) {
          this.animation = actorInstance._Animations.find(
            animation => animation._Name === this.animationName
          );
        } else {
